...
def getDFAData2(inputDir, keyList):
    """ Return list of files with key words  """
    fileList = [inputDir +
                str(name) for name in os.listdir(inputDir) if os.path.isfile(os.path.join(inputDir, name))]

    newFileList = fileList
    for subs in keyList:
        newFileList = [i for i in newFileList if subs in i]
...
if __name__ == "__main__":
    Dir = os. getcwd()
    inputDir = os.path.join(Dir, 'data', 'avaHockeySmoothChannel', 'Outputs', 'com1DFA', 'peakFiles')
    keyList = ['entres', 'pfd']
    newFileList = getDFAData2(inputDir, keyList)
    print(newFileList)
